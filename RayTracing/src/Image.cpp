...
void Image::set_pixel(const Color::Color& color, int x, int y)
{
...
void Image::gamma_correct(double gamma)
{
	for (auto& pixel : image_data)
		pixel = Color::gamma_correct(pixel, gamma);
}
...
void Image::to_ppm(const std::string& filename) const
{
	...
	for (const auto& pixel : image_data) {
		file << Color::arithmetic_to_true_color(pixel) << '\t';
		++col;
...
std::vector<unsigned char> Image::prepare_output() const
{
	...
	for (const auto& color : image_data) {
		Color::Color pixel_color = Color::arithmetic_to_true_color(color); 
...
Image Image::average_images(const std::vector<Image>& images)
{
	...
	for (const auto& image : images) {
		for (size_t i = 0; i < image.image_data.size(); i++) {
			result.image_data[i] += image.image_data[i] / float(images.size());
		}
	}
	return result;
}

const Color::Color& Image::get_pixel(int x, int y) const
{

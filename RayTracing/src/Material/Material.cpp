...
{
	Vec3D scatter_direction = hit_record.normal + Random::random_in_hemisphere(hit_record.normal, state);
...
{
	Vec3D scatter_direction = ray_in.direction().reflect(hit_record.normal) + fuzz * Random::random3d_in_unit_sphere(state);
...
Ray Dielectric::scatter(const Ray& ray_in, const HitRecord& hit_record, uint32_t& state) const
{
	...
	if (sin_theta * ri > 1 || reflection_factor > Random::random_double(state, 1)) { // total reflection
